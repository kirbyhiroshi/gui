...
async function fetchReports() {
    ...
    try {
        ...
        // 1. レスポンスオブジェクト全体をコンソールに出力
        console.log('✅ レスポンスオブジェクト全体:', response);

        // 2. 重要なプロパティを個別に確認
        console.log('HTTP ステータスコード:', response.status); // 200なら成功
        console.log('ステータスOK (200-299):', response.ok);   // trueなら成功
        console.log('Content-Type:', response.headers.get('Content-Type')); // 'application/json' であるべき

        // 3. レスポンスボディの内容をテキストとして取得（JSON解析前に確認）
        const responseText = await response.clone().text(); // .clone()でボディを保護してから取得
        console.log('レスポンスボディ (生テキスト):', responseText);
